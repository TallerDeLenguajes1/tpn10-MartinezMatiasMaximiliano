...
namespace Helpers
{
    class ConversorDeMorse
    {
        ...
        public static void CrearArchivoTAM(string textoT)
        {
            ...
            string DireMorse = @"C:\Repogit\tpn10-MartinezMatiasMaximiliano\MorseMP3\Morse\";
        ...
        {
            string Directorio = @"C:\Repogit\tpn10-MartinezMatiasMaximiliano\MorseMP3\Morse\";
        ...
        public static void CrearArchivoAudio(string Texto)
        {
            string Directorio = @"C:\Repogit\tpn10-MartinezMatiasMaximiliano\MorseMP3\Morse\";
            string texto = ConversorDeMorse.LeerArchivo(Texto);
            string fecha = DateTime.Now.ToString("MM-dd-yyyy HH,mm,ss");

            string archmp3 = $"{Texto}_[{fecha}].mp3";
            ...
            foreach (string palabra in PalabrasMorse)
            {
                ...
                using (FileStream stream = File.OpenWrite(Directorio + archmp3))
                {
                    foreach (char item in Letra)
                    {
                        if (item == '.')
                        {
                            byte[] punto = File.ReadAllBytes(@"C:\Repogit\tpn10-MartinezMatiasMaximiliano\MorseMP3\punto.mp3");
                            stream.Write(punto, 0, punto.Length);
                            stream.Flush();
                        }
                        else if (item == '-')
                        {
                            byte[] raya = File.ReadAllBytes(@"C:\Repogit\tpn10-MartinezMatiasMaximiliano\MorseMP3\raya.mp3");
